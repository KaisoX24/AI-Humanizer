...
if 'humanized_result' not in st.session_state:
    st.session_state.humanized_result = None
    
if 'text' not in st.session_state:
    st.session_state.text=None

st.session_state.uploaded_file=st.sidebar.file_uploader("Please Upload your file",type=['.txt','.pdf'])
...
if not st.session_state.uploaded_file:
    ...
    st.session_state.text=st.session_state.uploaded_file.read().decode('utf-8') if file_type=='txt' else extract_text(uploaded_file)
